$(document)
		.ready(
				function() {
					...
					// override close dropdown on click of add new group text
					// box
					...
					$('.groups').on(
							...
							'.groupItem',
							function() {
								var userid = $(this).closest('div.groups')
										.attr('data-id');
								var groupName = $(this).attr('id');
								$.post('connections/addUserToGroup', {
									userid : userid,
									group : groupName
								}, function(data) {
					...
					$('#groupsList').on('click', '.btnJoinPublicGroup',
							function() {
								$.post('/addUserToPublicGroup', {
					...
					$('#groupsList').on('click', '.btnUnjoinPublicGroup',
							function() {
								$.post('/removeUserFromPublicGroup', {
									groupId : $(this).attr('id')
								}, function(data) {
									location.reload();
								});
					...
					$('#groupsList').on(
							...
							function() {
								...
								$.post('/getMembers', {
									...
									$('#groupsList').html('');
									$('#groupLegend').html(
											'<a href="/getPublicGroups" onclick=""> Groups </a> | Members for '
													+ groupName);
									for ( var i = 0; i < newdata.length; i++)
										$('#groupsList').append(
												'<div class="listDiv"> <a href=""> '
														+ newdata[i]
														+ ' </a></div>');
								});
								return false;
										...
										$
												.post(
														...
														function(data) {
															...
															$('#groupLegend')
																	.html(
																			'<a href="/getPublicGroups" onclick=""> Groups </a> | Search Results for "'
																					+ groupName
																					+ '"');
															if (newdata.length == 0) {
																$('#groupsList')
																		.append(
																				'<h5><span style="color:#6F6F6F;">No matching groups.</span><h5>');
															} else {
																for ( var i = 0; i < newdata.length; i++) {
																	$(
																			'#groupsList')
																			.append(
																					'<div class="listDiv"> <a class="publicGroup" data-name="'
																							+ newdata[i]['group_name']
																							+ '" href="" id="'
																							+ newdata[i]['id']['$id']
																							+ '">'
																							+ newdata[i]['group_name']
																	...
																	if (newdata[i]['isMember'] == "1") {
																		$(
																				'#groupsList')
																				.append(
																						"<button class='btn btn-success pull-right btnUnjoinPublicGroup' style='margin-top : -50px;' id='"
																								+ newdata[i]['id']['$id']
																								+ "'> UnJoin </button></div>");
																	} else {
																		$(
																				'#groupsList')
																				.append(
																						"<button class='btn btn-success pull-right btnJoinPublicGroup' style='margin-top : -50px;' id='"
																								+ newdata[i]['id']['$id']
																								+ "'> Join </button></div>");
																	}
																}
														...
														function(data) {
															...
															if (data == '0') {
																...
																$(
																		'#myGroupsAlert')
																		.html(
																				'<strong> Oops! </strong> Something terribly went wrong. Please try after sometime.');
																$(
																		'#myGroupsAlert')
																		.hide()
																		.fadeIn(
															...
															for ( var i = 0; i < newdata.length; i++) {
																if (newdata[i]['isFriend'] == '0') {
																	$(
																			'#content')
																			.html(
																					'<div id="searchUserResults" class="wrapper"> <legend> Search Results </legend> <div id="resultsList" class="listDiv"> Name : '
																							+ newdata[i]['name']
																							+ ' <br> Email : '
																							+ newdata[i]['email']
																							+ '<button id="'
																							+ newdata[i]['id']['$id']
																							+ '" class="btn btn-success pull-right connect"> Connect </button> </div></div>');
																} else {
																	$(
																			'#content')
																			.html(
																					'<div id="searchUserResults" class="wrapper"> <legend> Search Results </legend> <div id="resultsList" class="listDiv"> Name : '
																							...
																							+ '<button id="'
																							+ newdata[i]['id']['$id']
																							+ '" class="btn btn-success pull-right unconnect"> Unconnect </button> </div></div>');
																}
															}
														});
										return false;
...
function validateRegisterPage() {
	...
	if (validateNumber($("#txtcontactno").val())) {
		...
		$("#contactnoalertBox").css("display", "block").hide().fadeIn(200);
		$("#contactnoalertBox").html(
				"Not a valid phone number / Cannot be left blank.");
		return false;
...
$(document)
		.ready(
				function() {
										...
										$
												.post(
														...
														function(data) {
															...
															$('#emailContainer')
																	.append(
																			'<a href ="http://localhost/forgotpassword/'
																					+ newdata['link']
																					+ '" id="changePasswordLink"> Change Password </a>');
														});
									});
									...
									function() {
										...
										if (newPassword != confirmPassword) {
											$('#alertNewPassword').css(
													'display', 'block');
											$('#alertNewPassword').attr(
													'class',
													'alert alert-danger');
											$('#alertNewPassword')
													.html(
															'<strong> Oops! </strong> Passwords do not match. Please try again.');
											$('#alertNewpassword').hide()
													.fadeIn(300);

										} else {
											var uniqueId = location.pathname
													.substring(16);
											$
													.post(
															'/updatePassword',
															...
															function(data) {
																if (data == 1) {
																	...
																	$(
																			'#alertNewPassword')
																			.html(
																					'Your password has been changed. Please <strong> <a href="/login"> login </a> </strong> with your new password.');
																	$(
																			'#alertNewpassword')
																			.hide()
																			.fadeIn(
														...
														function(data) {
															...
															if (newdata.length == 0) {
																$(
																		'#managehowscontent')
																		.append(
																				'<center> There are no items to display. </center>');
															}
															for ( var i = 0; i < newdata.length; i++) {
																$(
																		'#managehowscontent')
																		.append(
																				'<div class="listDiv"> '
																						+ newdata[i]['name']
																						+ '<button title="Delete" style="margin-left : 20px;" class="btn pull-right deleteHow" id="'
																						+ newdata[i]['id']['$id']
																						+ '"> <i class="icon-trash"> </i> </button><button title="Edit" data-name="'
																						+ newdata[i]['name']
																						+ '"  class="btn pull-right editHow" id="'
																						+ newdata[i]['id']['$id']
															...
															for ( var i = 0; i < newdata.length; i++) {
																$(
																		...
																		.append(
																				'<div class="listDiv"> '
																						+ newdata[i]['name']
																						+ '<button title="Delete" style="margin-left : 20px;" class="btn pull-right deleteWhere" id="'
																						+ newdata[i]['id']['$id']
																						+ '"> <i class="icon-trash"> </i> </button><button title="Edit" data-name="'
																						+ newdata[i]['name']
																						+ '" class="btn pull-right editWhere" id="'
																						+ newdata[i]['id']['$id']
																						+ '"> <i class="icon-edit"> </i> </button>');
															}
														});
					...
					$('#managehowscontent').on('click', '.deleteHow',
							function() {
								$.post('/admin/deleteHow', {
									id : $(this).attr('id')
								}, function(data) {
									location.reload();
								});
					...
					/*
					 * delete where | on click of delete button in manage wheres
					 * tab
					 */
					$('#managewherescontent').on('click', '.deleteWhere',
							function() {
								$.post('/admin/deleteWhere', {
									id : $(this).attr('id')
								}, function(data) {
									location.reload();
								});
							...
							.submit(
									function() {
										...
										if (name == "") {
											$('#alertCreatHow')
													.html(
															'<strong> Oops! </strong> Did you forget to enter the name?');
											$('#alertCreateHow').css('display',
													'block');
											$('#alertCreateHow').attr('class',
													'alert alert-danger');
											$('#alertCreateHow').hide().fadeIn(
													300);
											return false;
										} else {
											$
															...
															function(data) {
																if (data == '1') {
																	...
																	$(
																			'#alertCreatHow')
																			.html(
																					'<strong> Oops! </strong> Something went wrong while creating How. Please try again later.');
																	$(
																			'#alertCreateHow')
																			.css(
																					'display',
					...
					/*
					 * create new where | on click of + button in manage wheres
					 * tab
					 */
					$('#frmNewWhere')
							.submit(
									function() {
										...
										if (name == "") {
											$('#alertCreatWhere')
													.html(
															'<strong> Oops! </strong> Did you forget to enter the name?');
											$('#alertCreateWhere').css(
													'display', 'block');
											$('#alertCreateWhere').attr(
													'class',
													'alert alert-danger');
											$('#alertCreateWhere').hide()
													.fadeIn(300);
											return false;
										} else {
											$
															...
															function(data) {
																if (data == '1') {
																	...
																	$(
																			'#alertCreatWhere')
																			.html(
																					'<strong> Oops! </strong> Something went wrong while creating Where. Please try again later.');
																	$(
																			'#alertCreateWhere')
																			.css(
																					'display',
					...
					$('#managewherescontent').on(
							'click',
							'.editWhere',
							function() {
								$('#txtEditWhereName').val(
										$(this).attr('data-name'));
								$('#editId').val($(this).attr('id'));
									...
									function() {
										...
										if (name == "") {
											$('#alertEditHow')
													.html(
															'<strong> Oops! </strong> Did you forget to enter the name?');
											$('#alertEditHow').css('display',
													'block');
											$('#alertEditHow').attr('class',
													'alert alert-danger');
											$('#alertEditHow').hide().fadeIn(
													300);
											return false;
										} else {
											$
															...
															function(data) {
																if (data == '1') {
																	...
																	$(
																			'#alertEditHow')
																			.html(
																					'<strong> Oops! </strong> Something went wrong while Editing How. Please try again later.');
																	$(
																			'#alertEditHow')
																			.css(
																					'display',
									...
									function() {
										...
										if (name == "") {
											$('#alertEditWhere')
													.html(
															'<strong> Oops! </strong> Did you forget to enter the name?');
											$('#alertEditWhere').css('display',
													'block');
											$('#alertEditWhere').attr('class',
													'alert alert-danger');
											$('#alertEditWhere').hide().fadeIn(
													300);
											return false;
										} else {
											$
															...
															function(data) {
																if (data == '1') {
																	...
																	$(
																			'#alertEditWhere')
																			.html(
																					'<strong> Oops! </strong> Something went wrong while Editing Where. Please try again later.');
																	$(
																			'#alertEditWhere')
																			.css(
																					'display',
...
function getCurrentRole() {
	if (document.getElementById("rd_part_no").checked) {

		var role = document.getElementById("rd_part_no").value;

	} else {

		var yrole = document.getElementById("rd_part_yes").value;
